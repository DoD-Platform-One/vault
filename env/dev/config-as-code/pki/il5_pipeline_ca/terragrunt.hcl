...
inputs = {
  mount_path                = "pki/il5/pipeline"
  mount_type                = "pki"
  max_mount_ttl             = "94608000" # 3 years
  default_mount_ttl         = "94608000" # 3 years
  ca_csr_cn                 = "DoD P1 IL5 Pipeline CA"
  int_mount_path            = dependency.int_ca_mount.outputs.path
  crl_url                   = ["https://cubbyhole.cnap.dso.mil/v1/pki/il5/pipeline/crl"]
  ocsp_svrs                 = ["https://deathstar.cnap.dso.mil"]
  name                      = "il5-pipeline-leaf"
  allowed_domains           = ["pipeline.il5.dso.mil"]
